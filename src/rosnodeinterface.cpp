  ...
  namespace matlab
  {
    ...
    void NodeInterface::initialize(int argc, char ** argv)
    {
      try {
        ...
        ROS_INFO("** Starting the model \"clock2seconds\" **\n");

        // initialize the model which will initialize the publishers and subscribers
        rtmSetErrorStatus(clock2seconds_M, (NULL));
        clock2seconds_initialize();
    ...
    boolean_T NodeInterface::getStopRequestedFlag(void)
    {
      ...
      return (!(rtmGetErrorStatus(clock2seconds_M)
                == (NULL)));

#else

      return (!(rtmGetErrorStatus(clock2seconds_M)
                == (NULL)) || rtmGetStopRequested(clock2seconds_M));
    ...
    {
      mRunModel = (rtmGetErrorStatus(clock2seconds_M) ==
                   (NULL));
      while (mRunModel) {
        mBaseRateSem.wait();
